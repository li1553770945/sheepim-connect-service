	...
	for {
		...
		for clientID, lastPingTime := range s.ClientConnMap.lastPingTimeMap {
			if now.Sub(lastPingTime) > constant.HearBearTimeOut {
				...
				// 调用 online 服务更新客户端状态为离线
				_, err := s.OnlineClient.SetClientStatus(context.TODO(), &online.SetClientStatusReq{
					ClientId:       clientID,
					ServerEndpoint: s.Endpoint,
					IsOnline:       false,
				})
				if err != nil {
					hlog.Errorf("Failed to update online status for client %s: %v", clientID, err)
				}
			}
		}
